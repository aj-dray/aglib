...
export async function main(
  ...
  // What this agent offers, by its own option ids. Filled from `session/new`
  // through `onConfig` on every turn, and by `offered()` when something wants
  // the menu before there has been one.
  let published: readonly AcpConfigOption[] = [];
  /** Whether this agent has been asked what it offers. Reset when the agent changes. */
  let asked = false;

  /**
   * Ask the agent what it offers, without asking it anything else.
   *
   * ACP publishes options in the answer to `session/new`, and a session opens
   * on the first prompt — so a menu drawn before the first turn would be empty
   * exactly when the choice it offers is still open. `acpOptions` spawns,
   * handshakes, reads what came back and closes: no prompt, nothing billed.
   *
   * Paid for on the first command rather than on every start. A task on argv
   * and a pipe never open a menu, and a process and a handshake are not free.
   */
  const offered = async () => {
    asked = true;
    const row = rowFor(choice.agent);
    const answer = await acpOptions({ agent: { command: row.command, env: credentialsFor(row) }, sandbox });
    published = answer.ok ? answer.value : [];
    if (!answer.ok) (sink.status ?? sink.write)(`· ${choice.agent} did not open: ${answer.error.message}\n`);
  };
  ...
  for await (const input of options.turns ?? [task]) {
    // A command may want the menu and a turn never does, so the agent is asked
    // what it offers the first time one arrives.
    if (!asked && input.startsWith("/")) await offered();
    ...
    if (command.handled) {
      ...
      if (command.select) {
        ...
        if (command.select.agent) {
          ...
          first = true;
          // What the last agent offered is not what this one does, and this one
          // is asked when something wants the answer.
          published = [];
          asked = false;
        }
      }
 ...
 * answer a question, so a non-terminal takes the first row and says which on
 * the banner.
 *
