 ...
 *
 * So `/effort` drives Claude Code and finds nothing on Cursor, where a level is
 * part of the model (`grok-4.6[effort=high]`) and choosing one is `/model`.
 * That difference is the agent's, and saying it is better than flattening it.
 *
 * So there is one command and not one per axis. `/model`, `/mode` and `/effort`
 * covered three categories and missed Claude Code's `fast` and `agent`
 * entirely, which is what naming another product's vocabulary always costs: the
 * table is a guess, and the guess is stale the first time an agent ships an
 * option nobody here thought of. `/options` lists what this agent published,
 * whatever that is, and a number drills into one. Nothing to keep current.
 *
 ...
 *
 * Picking is two lines rather than a nested prompt: `/model` prints a numbered
 * list, and the next line chooses. A second readline over the same stdin while
 * the turn source owns it is a fight nobody needs for a menu.
 */
...
export function runCommand(input: {
  ...
  if (name === "options") {
    ...
    if (!published.length) {
      say(`${input.choice.agent} has published nothing yet — ask it something first, and it will say what it offers.`);
      return { handled: true };
    }
