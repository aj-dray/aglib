...
/**
 * Pi on the route Pi actually takes.
 *
 * These tests used to wrap a real OpenRouter model in a local server that
 * impersonated Anthropic and hand Pi the socket — translating an OpenAI-wire
 * provider into Anthropic frames to reach a loop that speaks the OpenAI wire
 * natively, and never once exercising the route the recipe gives it. Left over
 * from when everything went through that server for the Claude Code SDK's sake.
 */
